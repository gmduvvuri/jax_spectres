...
This project attempts to rewrite ``SpectRes`` with three major differences:

1. This project writes all code to be usable with ``JAX`` to benefit from that framework's list of features that speed up computation. The original package has a ``numba`` option for similar reasons, but this is not always compatible with fitting tools like ``NumPyro``, motivating this rewrite.
...
3. To simplify the JAX rewrite, the core resampling function assumes the input spectra are 1-D. Resampling higher dimensional spectra like a spectroscopic timeseries or spatial datacube should make use of explicitly batched/vmapped versions of the ``jax_spectres_lean`` function instead of assuming the function is agnostic to input dimension.

Installation
------------

``JAX_SpectRes`` can be installed using ``pip`` linked to the source GitHub repository.

.. code::

	pip install git+https://github.com/gmduvvuri/jax_spectres.git@main
